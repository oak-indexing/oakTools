...
runner.test('Should include jcr:primaryType property when only path restriction exists', function() {
    // Arrange
    const testSQL = `SELECT * FROM [cq:LiveSyncCancelled]\nWHERE ISDESCENDANTNODE('/content')`;
    ...
    const indexRoot = indexDef["/oak:index/cqLiveSyncCancelledLucene-12-custom-1"];
    const rules = indexRoot.indexRules["cq:LiveSyncCancelled"];
    const props = rules.properties;

    // Assert path restrictions propagated
    this.assertEqual(indexRoot.includedPaths, ["/content"], 'includedPaths should reflect path restriction');
    this.assertEqual(indexRoot.queryPaths, ["/content"], 'queryPaths should reflect path restriction');

    // The key is generated by naming rules; for "jcr:primaryType" it becomes "primaryType"
    this.assertEqual(props.hasOwnProperty('primaryType'), true, 'properties should include primaryType key');
    this.assertEqual(props['primaryType']['jcr:primaryType'], 'nt:unstructured', 'primaryType def primaryType');
    this.assertEqual(props['primaryType'].name, 'jcr:primaryType', 'primaryType def name');
    this.assertEqual(props['primaryType'].propertyIndex, true, 'primaryType def propertyIndex');
});
